...
exports.handleCompetitorRefreshJob = void 0;
...
const handleCompetitorRefreshJob = async (data) => {
    ...
    try {
        ...
        const competitorRes = await client.query(`SELECT * FROM competitors WHERE id = $1 AND organization_id = $2`, [data.competitorId, data.organizationId]);
        ...
        const updateRes = await client.query(`UPDATE competitors SET last_seen_at = NOW(), updated_at = NOW()
       WHERE id = $1 RETURNING *`, [competitor.id]);
        const updatedCompetitor = updateRes.rows[0];
        // 2. Take a snapshot
        await client.query(`INSERT INTO competitor_snapshots 
...
exports.handleCompetitorRefreshJob = handleCompetitorRefreshJob;
