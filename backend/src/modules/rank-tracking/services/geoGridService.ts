export interface GeoGridParams {
  latitude: number;
  longitude: number;
  gridSize: number;
  radiusMeters: number;
 ...
 */
function validateGridParams(params: GeoGridParams): void {
 ...
 */
function calculateOffsetCoordinate(
  lat: number,
  lon: number,
 ...
 */
export function generateGrid(params: GeoGridParams): GeoGridPoint[] {
  ...
  for (let row = 0; row < gridSize; row++) {
    for (let col = 0; col < gridSize; col++) {
      ...
      const { latitude: newLat, longitude: newLon } = calculateOffsetCoordinate(
        latitude,
        longitude,
        offsetLatMeters,
        offsetLonMeters
      ...
      const distanceFromCenterMeters = Math.sqrt(
        Math.pow(offsetLatMeters, 2) + Math.pow(offsetLonMeters, 2)
      );
