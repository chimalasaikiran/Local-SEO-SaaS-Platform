...
class BusinessRepository {
    static async list(organizationId, query) {
        ...
        const countResult = await db_1.default.query(`SELECT COUNT(*) FROM businesses b ${whereClause}`, values);
    ...
    static async findById(organizationId, businessId) {
        const result = await db_1.default.query(`SELECT * FROM businesses WHERE id = $1 AND organization_id = $2`, [businessId, organizationId]);
        return result.rows[0] || null;
    }
    static async findBySlug(organizationId, slug) {
        const result = await db_1.default.query(`SELECT * FROM businesses WHERE slug = $1 AND organization_id = $2`, [slug, organizationId]);
        return result.rows[0] || null;
    }
    static async create(organizationId, slug, payload) {
    ...
    static async update(organizationId, businessId, payload) {
        const setFields = [];
        const values = [];
        let paramIndex = 1;
